    ...
    <div className="px-6 lg:px-10 py-8 max-w-7xl mx-auto">
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-3">
        <Link href="/app/scripts/new" className="group relative overflow-hidden rounded-2xl border border-border bg-card p-6 transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-brand/5">
          ...
          <span className="mt-3 inline-block text-xs font-mono uppercase tracking-widest text-brand">Live now →</span>
        </Link>
        <Link href="/app/ideas" className="group relative overflow-hidden rounded-2xl border border-border bg-card p-6 transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-brand/5">
          ...
          <span className="mt-3 inline-block text-xs font-mono uppercase tracking-widest text-brand">{ideasCount > 0 ? `${ideasCount} in bank →` : "Live now →"}</span>
        </Link>
        <Link href="/app/thumbnails" className="group relative overflow-hidden rounded-2xl border border-border bg-card p-6 transition-all hover:-translate-y-0.5 hover:shadow-lg hover:shadow-brand/5">
          ...
          <span className="mt-3 inline-block text-xs font-mono uppercase tracking-widest text-brand">{thumbnailsCount > 0 ? `${thumbnailsCount} tested →` : "Live now →"}</span>
        </Link>
      </div>
      ...
      <div className="mt-12">
        <div className="flex items-end justify-between mb-4">
          ...
          {scripts.length > 0 && (
            <Button asChild variant="ghost" size="sm"><Link href="/app/scripts">View all<ArrowRight className="ml-1 h-4 w-4" /></Link></Button>
          )}
        </div>
        {scripts.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-border bg-card/50 p-12 text-center">
            <div className="inline-flex h-14 w-14 rounded-2xl brand-gradient items-center justify-center text-white mb-4"><Sparkles className="h-6 w-6" /></div>
            <h3 className="font-display text-lg font-bold">No scripts yet</h3>
            <p className="mt-1 text-sm text-muted-foreground max-w-sm mx-auto">Your first AI-generated script is one click away. It takes about 3 seconds.</p>
      ...
      <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-border bg-card p-5">
          <div className="flex items-center justify-between"><span className="text-xs font-mono uppercase tracking-widest text-muted-foreground">Scripts</span><PenLine className="h-4 w-4 text-brand" /></div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          <div className="flex items-center justify-between"><span className="text-xs font-mono uppercase tracking-widest text-muted-foreground">Ideas</span><Lightbulb className="h-4 w-4 text-brand-pink" /></div>
          <div className="mt-2 font-display text-3xl font-bold">{ideasCount}</div>
          <p className="mt-1 text-xs text-muted-foreground">{publishedCount > 0 ? `${publishedCount} published` : "in your bank"}</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-5">
          <div className="flex items-center justify-between"><span className="text-xs font-mono uppercase tracking-widest text-muted-foreground">Thumbnails</span><ImagePlus className="h-4 w-4 text-brand" /></div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          <div className="flex items-center justify-between"><span className="text-xs font-mono uppercase tracking-widest text-muted-foreground">Waitlist</span><TrendingUp className="h-4 w-4 text-brand-pink" /></div>
          <div className="mt-2 font-display text-3xl font-bold">{waitlistCount + 4200}</div>
